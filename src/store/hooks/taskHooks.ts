...
export const useTaskDelete = () => {
	const refecthQuery = useQueryClient();
	return useMutation({
		mutationFn: async (id: string) => await DeleteApi({ url: taskUrl + "/delete/" + id }),
		onSuccess: () => {
			refecthQuery.refetchQueries([taskUrl]);
		},
	});
...
export const useTaskList = (params?: TaskPagination) => {
	...
	return useQuery({
		queryFn: async () =>
			await GetApi<PaginationResponse<TaskFormType[]>, TaskPagination>({ url: taskUrl + "/", params }),
...
export const useTaskListView = (id?: string | null) => {
	return useQuery({
		queryFn: async () => await GetApi<TaskFormType>({ url: taskUrl + "/view" + `/${id}` }),
...
export const useStatusUpdate = (id?: string | null) => {
	...
	return useMutation({
		mutationFn: async (formData: TaskFormType) =>
			await PostApi<TaskFormType>({ url: taskUrl + "/statusupdate" + `/${id}`, formData }),
		onSuccess: (res) => {
...
export const useCloseTask = () => {
	const refecthQuery = useQueryClient();
	return useMutation({
		mutationFn: async (id: string) => PostApi<TaskFormType>({ url: taskUrl + "/taskclose" + `/${id}` }),
		onSuccess: (res) => {
			refecthQuery.refetchQueries([taskViewKey, String(res.data.id)]);
			refecthQuery.refetchQueries({
				queryKey: [taskUrl],
